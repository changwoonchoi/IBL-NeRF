...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_type", required=True, type=str, choices=["mitsuba", "colmap"])
    args = parser.parse_args()
    metrics = calculate_metrics(args.gt, args.pred, args.dataset_type)
    print(f"ssim: {np.mean(np.asarray(metrics['ssim']))}")
    print(f"psnr: {np.mean(np.asarray(metrics['psnr']))}")
    print(f"mse: {np.mean(np.asarray(metrics['mse']))}")
